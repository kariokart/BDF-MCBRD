...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Kari\\Downloads\\dynamic_selective_inspect-master (1)\\dynamic_selective_inspect-master\\BDF MCBRD.py',
...
Informed_ConsentClock = core.Clock()
...
# Initialize components for Routine "text_input"
text_inputClock = core.Clock()
text_3 = visual.TextStim(win=win, name='text_3',
    text='Type your understanding of the story below. \nPress [Enter] when you are done.',
    font='Times New Roman',
...
for thisComponent in Informed_ConsentComponents:
...
Informed_ConsentClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "Informed_Consent"-------
while continueRoutine:
    # get current time
    t = Informed_ConsentClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=Informed_ConsentClock)
...
key_resp_2.rt = []
_key_resp_2_allKeys = []
mouseVis = event.Mouse(win=win)
mouseVis.setVisible(False)
# keep track of which components have finished
InstructionsComponents = [Instruction, mouse_2, key_resp_2]
...
for thisTrial in trials:
    ...
    # ------Prepare to start Routine "text_input"-------
    continueRoutine = True
    # update component parameters for each repeat
    modify = False
    text.text = ''
    ...
    # keep track of which components have finished
    text_inputComponents = [text_3, text]
    for thisComponent in text_inputComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    text_inputClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "text_input"-------
    while continueRoutine:
        # get current time
        t = text_inputClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=text_inputClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        if int(EndOfStory) == 1:
    ...
    # the Routine "text_input" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
